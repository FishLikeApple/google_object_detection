...
class CascadeRCNN(BaseDetector, RPNTestMixin):
    ...
    def forward_train(self,
        ...
        if self.with_rpn:
            ...
            x_test.append(rpn_outs) 
            rpn_loss_inputs = rpn_outs + (gt_bboxes, img_meta,
                                          self.train_cfg.rpn)
            x_test.append(rpn_loss_inputs)
            rpn_losses = self.rpn_head.loss(
                *rpn_loss_inputs, gt_bboxes_ignore=gt_bboxes_ignore)
            x_test.append(rpn_losses)
            losses.update(rpn_losses)

            proposal_cfg = self.train_cfg.get('rpn_proposal',
                                              self.test_cfg.rpn)
            proposal_inputs = rpn_outs + (img_meta, proposal_cfg)
            x_test.append(proposal_inputs)
            proposal_list = self.rpn_head.get_bboxes(*proposal_inputs)
        else:
            proposal_list = proposals

        # save log
        #if (self.forward_counting >= 500 and self.forward_counting <= 520) or (self.forward_counting <= 20):
        #    with open('/cos_person/275/1745/object_detection/output/latertensorlog'+str(self.forward_counting), 'wb') as f:
        #        pickle.dump(x_test, f)
        #self.forward_counting += 1
